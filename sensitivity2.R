...
afinal01 <- afinalb[c('Reference.Key.','Rxst','Rxed','DOB','sdate','edate1','admdt','Sex.','DOD')] 
...
#add first MPH or first ADHD as new obs start
# mph9320 <- readRDS("M:/Cohort Raw Data (do not edit)/TIMESPAN/3.combine/mph9320.rds")
mph1 <- a %>% 
  mutate(mph1st=if_else(is.na(Rxst),disdt,Rxst)) %>% 
  select(Reference.Key.,mph1st)%>% 
...
adhd1 <- rbind(adhd_mph_ip2,adhd_mph_alldx1) %>% 
...
afinal1_merge <- merge(afinal1,mph_adhd,by="Reference.Key.") %>% 
  mutate(sdate1=pmax(sdate,adhd_date,na.rm = T))
...
afinal2 <- afinal1_merge %>% 
  mutate(DOB=as.Date(DOB),
         ...
         sdate=as.numeric(difftime(sdate1, DOB, unit="days") +1),
