...
afinalb <- afinal %>% 
  filter(admdt >= sdate,admdt <= edate1) %>% 
  mutate(du = Rxst-sdate+1,admdt=as.Date(admdt,origin="1970-01-01"))
...
afinal1 <- afinal1 %>% mutate(du=as.numeric(Rxed - Rxst+1))
...
include <- afinal2 %>% 
  mutate(du=Rxed-Rxst+1) %>% 
  group_by(Reference.Key.) %>% 
  mutate(du_sum=sum(du)) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  select(Reference.Key.,du_sum) %>% 
  filter(du_sum>=120)
...
test <- afinal2 %>% 
  filter(Reference.Key.%in%include$Reference.Key.) %>% 
  arrange(Reference.Key.,Rxst)
