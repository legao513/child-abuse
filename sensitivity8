...
#find first psy dx
psy_dx <- rbind(adhd_mph_ip2,adhd_mph_alldx1) %>% 
  filter(grepl('^29[0-9]|^30[0-9]|^31[0-3]|^31[5-9]',All.Diagnosis.Code..ICD9..)==T) %>% 
  arrange(Reference.Key.,admdt) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  filter(Reference.Key.%in%afinal1$Reference.Key.) %>%
  mutate(psy_date=admdt) %>% 
  select(Reference.Key.,psy_date)

afinal1_psy <- merge(afinal1,psy_dx,all = T) %>% 
  mutate(psyst=if_else(psy_date<=edate1,psy_date,as.Date(NA)),
         psyed=if_else(psy_date<=edate1,edate1,as.Date(NA))) %>% 
  select(Reference.Key.,DOB,sdate,edate1,admdt,Sex.,DOD,
         Rxst0,Rxed0,Rxst1,Rxed1,Rxst2,Rxed2,psyst,psyed,Rxst,Rxed) %>% 
  mutate(psyrxst=NA, psyrxed=NA)

psy_rx <- adhd_mph_allrx1 %>% 
  filter(grepl('^4.[1238]',Therapeutic.Classification..BNF..)==T,
         Reference.Key.%in%afinal1$Reference.Key.,
         !is.na(Prescription.Start.Date.)) %>% 
  mutate(Prescription.Start.Date.=as.Date(Prescription.Start.Date.),
         Prescription.End.Date.=as.Date(Prescription.End.Date.)) %>% 
  select(Reference.Key.,Prescription.Start.Date.,Prescription.End.Date.)

psyoverlapping <- psy_rx %>%
  arrange(Reference.Key., Prescription.Start.Date., Prescription.End.Date.) %>% # as suggested by @Jonno in case the data is unsorted
  group_by(Reference.Key.) %>%
  mutate(indx = c(0, cumsum(as.numeric(lead(Prescription.Start.Date.)-1) >
                              cummax(as.numeric(Prescription.End.Date.)))[-n()])) %>%
  group_by(Reference.Key., indx) %>%
  summarise(psyrxst = first(Prescription.Start.Date.), psyrxed = max(Prescription.End.Date.)) %>% 
  select(-indx) %>% 
  mutate(Rxst0=NA,Rxed0=NA,Rxst1=NA,Rxed1=NA,Rxst2=NA,Rxed2=NA,Rxst=NA,Rxed=NA)

afinal1_psy_fix <- afinal1_psy %>% 
  select(Reference.Key.,DOB,sdate,edate1,admdt,Sex.,DOD,
         psyst,psyed) %>% 
  filter(duplicated(Reference.Key.)==F)
psy_rx_all <- merge(psyoverlapping,afinal1_psy_fix,by="Reference.Key.",all.x = T) %>% 
  select(colnames(afinal1_psy))

afinal_all <- bind_rows(psy_rx_all,afinal1_psy)

fm <- readRDS("M:/Personal/Gao Le/sens_psy_rx.rds") %>%
  select(reference_key,rxst90b,rxed90b,rxst90a,rxed90a,rxstsub,rxedsub,
         rx_st_num,rx_ed_num,obst,obed,dob,dod)
  group_by(indiv)%>%
  mutate(len_f=length(indiv)) %>%
  select(indiv,len_f)
# length(unique(fm$reference_key))
# test <- formatdata_all2 %>% 
#   group_by(indiv)%>% 
#   mutate(len_g=length(indiv)) %>% 
#   select(indiv,len_g)
# fm_test <- merge(fm,test,by="indiv") %>% 
#   mutate(dif=len_f-len_g)
#11 SCCS analysis
#add covid index

afinal2 <- afinal_all %>% 
  mutate(DOB=as.Date(DOB),
         Rxst=ifelse(!is.na(Rxst),as.numeric(difftime(Rxst, DOB, unit="days") +1),NA),
         Rxed=ifelse(!is.na(Rxed),as.numeric(difftime(Rxed, DOB, unit="days") +1),NA),
         Rxst0=ifelse(!is.na(Rxst0),as.numeric(difftime(Rxst0, DOB, unit="days") +1),NA),
         Rxed0=ifelse(!is.na(Rxed0),as.numeric(difftime(Rxed0, DOB, unit="days") +1),NA), 
         Rxst1=ifelse(!is.na(Rxst1),as.numeric(difftime(Rxst1, DOB, unit="days") +1),NA),
         Rxed1=ifelse(!is.na(Rxed1),as.numeric(difftime(Rxed1, DOB, unit="days") +1),NA),
         Rxst2=ifelse(!is.na(Rxst2),as.numeric(difftime(Rxst2, DOB, unit="days") +1),NA),
         Rxed2=ifelse(!is.na(Rxed2),as.numeric(difftime(Rxed2, DOB, unit="days") +1),NA),
         psyst=ifelse(!is.na(psyst),as.numeric(difftime(psyst, DOB, unit="days") +1),NA),
         psyed=ifelse(!is.na(psyed),as.numeric(difftime(psyed, DOB, unit="days") +1),NA),
         psyrxst=ifelse(!is.na(psyrxst),as.numeric(difftime(psyrxst, DOB, unit="days") +1),NA),
         psyrxed=ifelse(!is.na(psyrxed),as.numeric(difftime(psyrxed, DOB, unit="days") +1),NA),
         sdate=ifelse(!is.na(sdate),as.numeric(difftime(sdate, DOB, unit="days") +1),NA),
         edate1=ifelse(!is.na(edate1),as.numeric(difftime(edate1, DOB, unit="days") +1),NA),
         admdt=ifelse(!is.na(admdt),as.numeric(difftime(admdt, DOB, unit="days") +1),NA),
...
test <- afinal2 %>% arrange(Reference.Key.,Rxst,psyrxst)
...
formatdata1 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2,psyst,psyrxst,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
                          aedrug=list(Rxed0,Rxed1,Rxed2,psyed,psyrxed,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
...
tapply(formatdata_all2$event, formatdata_all2$psyst, sum)
tapply(formatdata_all2$interval, formatdata_all2$psyst, sum)/365.25


fmla <- paste(event~Rxst0+Rxst1+Rxst2+psyst+psyrxst+c_i_under40_s+c_i_40to50_s+c_i_50to60_s+c_i_60to70_s+c_i_up70_s+age+season, "+", "strata(indiv)", 
              "+", "offset(log(interval))")
