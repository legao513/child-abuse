...
abc2 <- merge(a0_1,HA_first,by="Reference.Key.") %>% 
  mutate(s=ymd('2001-01-01'),e=ymd('2019-12-31')) %>% 
...
formatdata1 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2),
                          aedrug=list(Rxed0,Rxed1,Rxed2),
...
formatdata_all2 <- rbind(formatdata1,formatdata2,formatdata3,formatdata4) 
...
fmla <- paste(event~Rxst0+Rxst1+Rxst2+age+season, "+", "strata(indiv)", 
              "+", "offset(log(interval))")
...
fmla <- paste(event~rx_fac+age+season, "+", "strata(indiv)", 
              "+", "offset(log(interval))")
fmla1 <- as.formula(paste("event~", fmla[3]))
library(survival)
