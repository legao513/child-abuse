...
mph_rx_first <- a %>%
  ...
  filter(grepl('METH',Drug.Name.)==T) %>% 
  arrange(Reference.Key.,mph1st) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  select(Reference.Key.,mph1st)

afinal1_incident <- merge(afinal1,mph_rx_first,by="Reference.Key.",all.x = T) %>% 
  filter(mph1st>=sdate)
...
afinal2 <- afinal1 %>% 
  filter(Reference.Key.%in%afinal1_incident$Reference.Key.) %>% 
