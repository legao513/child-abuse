...
bc1_1 <- rbind(adhd_mph_ip2,adhd_mph_alldx1) %>% 
  ...
  arrange(Reference.Key.,admdt) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  filter(Reference.Key.%in%a9_5$Reference.Key.)



afinal_2 <- merge(a9_5,bc1_1,by ="Reference.Key.",all.x=T)%>% 
...
dosage_data <- afinal_2 %>% 
  select(Reference.Key.,Drug.Frequency.,Drug.Frequency.Fix,Dosage.,Sex.,Drug.Strength.)
...
summary(dosage_data$dosage_cal[which(!is.na(dosage_data$dosage_cal))])
summary(dosage_data$dosage_cal[which((!is.na(dosage_data$dosage_cal)&dosage_data$Sex.=="M"))])
summary(dosage_data$dosage_cal[which((!is.na(dosage_data$dosage_cal)&dosage_data$Sex.=="F"))])

dosage_data1 <- dosage_data %>% 
  filter(!is.na(dosage_cal)) %>% 
  mutate(dosage_cal1=ifelse(as.numeric(dosage_cal)>10,1,2)) %>% 
  group_by(Reference.Key.) %>% 
  mutate(ct_len=length(unique(dosage_cal1))) %>% 
  select(Reference.Key.,ct_len,dosage_cal1)%>% 
  filter(duplicated(Reference.Key.)==F) 
table(dosage_data1$ct_len)



#baseline duration
duration_data <- afinal_2 %>% 
  select(Reference.Key.,Rxst,Rxed,Sex.) %>% 
  mutate(du=as.numeric(Rxed-Rxst))
summary(duration_data$du)
summary(duration_data$du[which(duration_data$Sex.=="M")])
summary(duration_data$du[which(duration_data$Sex.=="F")])




#8 solve the overlapping of exposure time----
aoverlapping <- afinal_2 %>%
  arrange(Reference.Key., Rxst, Rxed) %>% # as suggested by @Jonno in case the data is unsorted
  group_by(Reference.Key.) %>%
  mutate(indx = c(0, cumsum(as.numeric(lead(Rxst)-1) >
                              cummax(as.numeric(Rxed)))[-n()])) %>%
  group_by(Reference.Key., indx) %>%
  summarise(start = first(Rxst), end = max(Rxed))
...
a10_2 <- afinal_2 %>% 
  select(Reference.Key.,DOB,sdate,edate1,admdt,Drug.Name.,Dosage.,Drug.Frequency.,Drug.Strength.,DOD,Sex.,
         Drug.Frequency.Fix) %>% 
  filter(duplicated(Reference.Key.) == FALSE)
afinal <- merge(aoverlapping,a10_2,by="Reference.Key.",all.x = TRUE)
colnames(afinal)[3] <- 'Rxst'
colnames(afinal)[4] <- 'Rxed' 
length(unique(afinal$Reference.Key.))#1123
...
afinalb <- afinal %>% 
  ...
  mutate(Rxst=if_else(Rxst<sdate,sdate,Rxst),
         ...
         du = Rxst-sdate,admdt=as.Date(admdt,origin="1970-01-01"))
length(unique(afinalb$Reference.Key.)) #1123
...
afinal01 <- afinalb[c('Reference.Key.','Rxst','Rxed','DOB','sdate','edate1','admdt','Sex.','DOD','Drug.Frequency.Fix','Drug.Frequency.','Dosage.','Drug.Strength.')] 
...
afinal1 <- afinal1 %>% mutate(cutoff90 = Rxst+89,ID = NA)
afinal1$ID <- 1:nrow(afinal1)
for(i in unique(afinal1$Reference.Key.)){
  exref <- afinal1 %>% filter(Reference.Key.== i)
  print(i)
  for(j in exref$ID[-length(exref$ID)]){
    print(j)
    afinal1[which(afinal1$ID == j+1),"cutoff90"]<-afinal1[which(afinal1$ID == j),"cutoff90"]
  }
}
...
dosage_data$dosage_cal <- as.numeric(dosage_data$tabno)*as.numeric(dosage_data$mg)*as.numeric(dosage_data$fre1)
summary(dosage_data$dosage_cal[which(!is.na(dosage_data$dosage_cal))])
summary(dosage_data$dosage_cal[which((!is.na(dosage_data$dosage_cal)&!is.na(dosage_data$Rxst1)))])
summary(dosage_data$dosage_cal[which((!is.na(dosage_data$dosage_cal)&!is.na(dosage_data$Rxst2)))])

dosage_data1 <- dosage_data %>% 
  # mutate(dosage_cal1=ifelse(as.numeric(dosage_cal)>10,1,2)) %>% 
  filter(Reference.Key.%in%recurrent_ref$Reference.Key.)%>% 
  group_by(Reference.Key.) %>% 
  mutate(ct_len=length(unique(dosage_cal1))) %>% 
  select(Reference.Key.,ct_len,dosage_cal1)%>% 
  filter(duplicated(Reference.Key.)==F) 
...
ttt1 <- dosage_data %>% 
  filter(dosage_cal>10)
ttt2 <- dosage_data %>% 
  filter(dosage_cal<=10)

ttt3 <- ttt1 %>% 
  filter(Reference.Key.%in%ttt2$Reference.Key.)

#died patients
died_pp <- afinal1 %>% 
  filter(!is.na(DOD))
length(unique(died_pp$Reference.Key.))



#11 SCCS analysis
#add covid index
covid_index <- read_xlsx("D:/OneDrive - connect.hku.hk/Other-share/5.Phd project/8.MPH_abuse/4.updated_analysis/COVID_index_HK.xlsx")

covid_index <- covid_index %>%
  mutate(Day=as.Date(Day)) %>% 
  filter(Day<= ymd('2020-12-31')) %>% 
  mutate(index_grp=cut(as.numeric(stringency_index),breaks=c(0,40,50,60,70,500),right=F,
                   labels = c("[0-40)","[40-50)","[50-60)","[60-70)","[70+]")),
         ee=Day)

indexoverlapping <- covid_index %>%
  arrange(index_grp, Day) %>% # as suggested by @Jonno in case the data is unsorted
  group_by(index_grp) %>%
  mutate(indx = c(0, cumsum(as.numeric(lead(Day)-1) >
                              cummax(as.numeric(ee)))[-n()])) %>%
  group_by(index_grp, indx) %>%
  summarise(start = first(Day), end = max(ee))


#plot of neglect and non-fill
test <- afinal1 %>% 
  filter(admdt>=Rxst,admdt<=Rxed) %>% 
  mutate(difff=as.numeric(Rxed-admdt))
library(ggplot2)
ggplot(data=test,aes(difff)) + 
  geom_bar(width=1)+
  theme(
  ...
  xlim(0,100)+
  xlab("Time since outcome to the end of the exposure \n (outcome happend during treatment period n=231)")

test1 <- afinal1 %>% 
  filter(!Reference.Key.%in%test$Reference.Key.,
         Rxst>admdt) %>%
  arrange(Reference.Key.,Rxst) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  mutate(difff=as.numeric(admdt-Rxst))
# library(ggplot2)
ggplot(data=test1,aes(difff)) + 
  geom_bar(width=1)+
  xlim(-100,0)+
  theme(
    axis.text.x = element_text(size = 18,face = "bold"),
  ...
  xlab("Time since outcome to the start of next exposure \n (outcome happend during non-treatment period n=825)")


test11 <- afinal1 %>% 
  filter(!Reference.Key.%in%test$Reference.Key.,
         Rxed<admdt) %>%
  arrange(Reference.Key.,desc(Rxst)) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  mutate(difff=as.numeric(admdt-Rxed)) %>% 
  filter(difff<=90)
# library(ggplot2)
ggplot(data=test11,aes(difff)) + 
  geom_bar(width=1)+
  xlim(0,100)+
  theme(
    axis.text.x = element_text(size = 18,face = "bold"),
  ...
  xlab("Time since end of previous exposure to outcome \n (outcome happend during non-treatment period n=53)")
...
test2 <- afinal1 %>% 
  filter(!Reference.Key.%in%test$Reference.Key.,
         !Reference.Key.%in%test_first$Reference.Key.,
         Rxst>admdt) %>%
  arrange(Reference.Key.,Rxst) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  mutate(difff=as.numeric(admdt-Rxst))
# library(ggplot2)
ggplot(data=test2,aes(difff)) + 
  geom_bar(width=1)+
  xlim(-100,0)+
  theme(
    axis.text.x = element_text(size = 18,face = "bold"),
  ...
  xlab("Time since outcome to the start of next exposure \n (outcome happend during post-treatment period n=95)")
...
afinal2 <- afinal1 %>% 
  mutate(DOB=as.Date(DOB),
         ...
         sdate=as.numeric(difftime(sdate, DOB, unit="days") +1),
...
#histogram plot
test_histogram <- afinal1 %>% 
  mutate(age=as.numeric((admdt-DOB)/365.25)) %>% 
  select(Reference.Key.,age,Sex.) %>% 
  filter(duplicated(Reference.Key.)==F)
summary(test_histogram$age)
hist(test_histogram$age,
     main="Distribution of age at event",
     xlab="Age at event",
     xlim=c(4,18),
     ylim = c(0,200))

#baseline age
baseline_age <- afinal1 %>% 
  mutate(age=as.numeric((sdate-DOB)/365.25)) %>% 
  select(Reference.Key.,age,Sex.) %>% 
  filter(duplicated(Reference.Key.)==F)

mean(baseline_age$age)
sd(baseline_age$age)
mean(baseline_age$age[which(baseline_age$Sex.=="M")])
mean(baseline_age$age[which(baseline_age$Sex.=="F")])
sd(baseline_age$age[which(baseline_age$Sex.=="M")])
sd(baseline_age$age[which(baseline_age$Sex.=="F")])


mean(baseline_age$age[which(baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)])
mean(baseline_age$age[which(!baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)])
sd(baseline_age$age[which(baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)])
sd(baseline_age$age[which(!baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)])

t.test(baseline_age$age[which(baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)],
       baseline_age$age[which(baseline_age$Reference.Key.%in%afinal2$Reference.Key.&!baseline_age$Reference.Key.%in%recurrent_exp_ref$Reference.Key.)])


#median follow up
meidan_fu <- afinal1 %>% 
  mutate(fu=(edate1-sdate)/365.25) %>%
  select(Reference.Key.,fu,Sex.) %>% 
  filter(duplicated(Reference.Key.)==F)
mean(meidan_fu$fu)
sd(meidan_fu$fu)


#median sum duration of mph
meidan_du <- afinal1 %>% 
  mutate(du=as.numeric(Rxed-Rxst)/365.25) %>%
  group_by(Reference.Key.) %>% 
  mutate(du_sum=sum(du)) %>% 
  select(Reference.Key.,du_sum,Sex.) %>% 
  filter(duplicated(Reference.Key.)==F)
mean(meidan_du$du_sum)
sd(meidan_du$du_sum)



# data_dob <- unique(test[,c("Reference.Key.","DOB")])
# dob <- as.numeric(format(data_dob$DOB,"%d%m%Y"))
...
# testdata <- merge(data1,data.frame(Reference.Key.=unique(data1$Reference.Key.),id=1:length(unique(data1$Reference.Key.))))%>%
#   select(id,Rxst0,Rxed0,Rxst1,Rxed1,Rxst2,Rxed2,DOB,admdt,sdate,edate1)
...
formatdata1 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
                          aedrug=list(Rxed0,Rxed1,Rxed2,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
...
formatdata2 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
                          aedrug=list(Rxed0,Rxed1,Rxed2,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
...
formatdata3 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
                          aedrug=list(Rxed0,Rxed1,Rxed2,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
...
formatdata4 <- formatdata(indiv=Reference.Key., astart=sdate, aend=edate1, aevent=admdt, 
                          adrug=list(Rxst0,Rxst1,Rxst2,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
                          aedrug=list(Rxed0,Rxed1,Rxed2,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
...
tapply(formatdata_all2$event, formatdata_all2$c_i_under40_s, sum)
tapply(formatdata_all2$interval, formatdata_all2$c_i_under40_s, sum)/365.25

tapply(formatdata_all2$event, formatdata_all2$c_i_40to50_s, sum)
tapply(formatdata_all2$interval, formatdata_all2$c_i_40to50_s, sum)/365.25

tapply(formatdata_all2$event, formatdata_all2$c_i_50to60_s, sum)
tapply(formatdata_all2$interval, formatdata_all2$c_i_50to60_s, sum)/365.25

tapply(formatdata_all2$event, formatdata_all2$c_i_60to70_s, sum)
tapply(formatdata_all2$interval, formatdata_all2$c_i_60to70_s, sum)/365.25

tapply(formatdata_all2$event, formatdata_all2$c_i_up70_s, sum)
tapply(formatdata_all2$interval, formatdata_all2$c_i_up70_s, sum)/365.25
...
eventdepenexp(indiv=Reference.Key., astart=sdate, aend=edate1,
              aevent=admdt, sameexpopar = F,
              adrug=cbind(Rxst0,Rxst1,Rxst2,c_i_under40_s,c_i_40to50_1_s,c_i_40to50_2_s,c_i_40to50_3_s,c_i_50to60_1_s,c_i_50to60_2_s,c_i_50to60_3_s,c_i_50to60_4_s,c_i_60to70_1_s,c_i_60to70_2_s,c_i_60to70_3_s,c_i_up70_s),
              aedrug=cbind(Rxed0,Rxed1,Rxed2,c_i_under40_e,c_i_40to50_1_e,c_i_40to50_2_e,c_i_40to50_3_e,c_i_50to60_1_e,c_i_50to60_2_e,c_i_50to60_3_e,c_i_50to60_4_e,c_i_60to70_1_e,c_i_60to70_2_e,c_i_60to70_3_e,c_i_up70_e),
              # agegrp = cumsum(c(366+365+365+365+366+365+1,365,365,366,365,365,365,366,365,365,365)),
              dataformat="multi", data=test)

eventdepenexp(indiv=case, astart=sta, aend=end,
              aevent=intus, adrug=cbind(cbind(rv,rvd2)),
              aedrug=cbind(cbind(rv+21,rvd2+21)),
              sameexpopar=F, dataformat="multi",
              data=rotdat)
...
#calculate e value
library(EValue)
evalues.RR(est = 0.26, lo = 0.17, hi = 0.41)
evalues.RR(est = 0.20, lo = 0.14, hi = 0.29)
evalues.RR(est = 0.14, lo = 0.11, hi = 0.18)


#subgroup by sex
test_M <- test %>% filter(Sex.=="M")
test_F <- test %>% filter(Sex.=="F")

mod <- clogit(formula = fmla1, data = formatdata_all2 %>% filter(indiv%in%test_M$Reference.Key.))
summary(mod)
mod <- clogit(formula = fmla1, data = formatdata_all2 %>% filter(indiv%in%test_F$Reference.Key.))
summary(mod)

#py by sex
dd1 <- formatdata_all2 %>% 
  filter(indiv%in%test_F$Reference.Key.)


tapply(dd1$event, dd1$Rxst0, sum)
tapply(dd1$interval, dd1$Rxst0, sum)/365.25

tapply(dd1$event, dd1$Rxst1, sum)
tapply(dd1$interval, dd1$Rxst1, sum)/365.25

tapply(dd1$event, dd1$Rxst2, sum)
tapply(dd1$interval, dd1$Rxst2, sum)/365.25
...
mod <- clogit(formula = fmla1, data = formatdata_all3%>% filter(indiv%in%test_M$Reference.Key.))
summary(mod)

mod <- clogit(formula = fmla1, data = formatdata_all3%>% filter(indiv%in%test_F$Reference.Key.))
summary(mod)
...
mod1_pre1 <- nonparasccs(indiv = Reference.Key., astart = sdate, aend = edate1, aevent = admdt,
                        adrug = Rxst, aedrug = Rxst+200, data = test)
f1 <- mod1_pre1
f1$timesinceexpo <- 0:200


l.plot.nonparasccs <- function(x, type="l", conf.int=0.95, ...) {
  ...
  plot(fit$ageaxis, fit$age, type=type, ylab = "relative incidence", xlab = "age (days)",cex.main=1.5, cex.lab=1.8, cex.axis=1.8)
  
  # Exposure related relative incidence function
  plot(fit$timesinceexpo, fit$exposure, type=type, ylab= "relative incidence", xlab= "days since start of treatment initiation", ylim = c(0, (max(uci)+1)),cex.main=1.5, cex.lab=1.8, cex.axis=1.8)
  abline(v=0, col="darkgoldenrod3")
  abline(h=1, col="lightskyblue3",lty=2)
...
l.plot.nonparasccs(f1)



#smooth
sp1 <- smoothexposccs(indiv=Reference.Key., astart=sdate, aend=edate1,
                      aevent=admdt, adrug=Rxst-200, aedrug=Rxst+200, 
                      agegrp = c(2193,2558,2923,3289,3654,4019,4384,4750,5115,5480,5845),
                      data=test)

l.plot.smoothexposccs <- function(x, type="l", conf.int=0.95, ...) {
  ...
  # Exposure related relative incidence function
  plot(fit$timesinceexpo, rho, type=type, ylab= "relative incidence", xlab= "days since start of treatment initiation", ylim = c(0, (max(uci)+1)),cex.main=1.8, cex.lab=1.8, cex.axis=1.5)
  abline(v=0, col="darkgoldenrod3")
  abline(h=1, col="lightskyblue3",lty=2)
...
sp2 <- sp1
sp2$timesinceexpo <--200:200
l.plot.smoothexposccs(sp2)
...
adhd <- adhd_mph_alldx %>% 
  filter(grepl("^314",All.Diagnosis.Code..ICD9..)==T,
         Reference.Key.%in%afinal1$Reference.Key.) %>% 
  arrange(Reference.Key.,as.Date(Reference.Date.)) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  select(Reference.Key.,Reference.Date.)
length(unique(adhd$Reference.Key.))

adhd_age <- merge(afinal1 %>% select(Reference.Key.,DOB) %>% filter(duplicated(Reference.Key.)==F),adhd,by="Reference.Key.",all.y =T) %>% 
    mutate(age=as.numeric(as.Date(Reference.Date.)-as.Date(DOB))/365.25)
summary(adhd_age$age)

icd <- list(c("stress","^308"),
            c("adjust","^309"),
            c("anxiety","^293.84|^300"),
            c("asd","^299"),
            c("depress","^296.[23]|^311"),
            c("conduct","^312"),
            c("emotion","^313"),
            c("intellectual","^31[789]"),
            c("development","^315"),
            c("other","^29[0-9]|^30[0-9]|^31[0-3]|^31[5-9]"))
...
com_fun <- function(z){
  ...
  d2 <- adhd_mph_alldx %>%
    filter(grepl(y,All.Diagnosis.Code..ICD9..)==T,
           Reference.Key.%in%adhd$Reference.Key.) %>% 
    filter(duplicated(Reference.Key.)==F)
  
  ddd <- data.frame("No_all"=nrow(d1),
                    "No_in_adhd"=nrow(d2)) %>% 
    mutate(disease=x,
           no_all_per=No_all/1124*100,
           No_in_adhd_per=No_in_adhd/922*100)
  return(ddd)
  
}

com_all <- bind_rows(lapply(icd,com_fun))
...
#Calculate the overall incidence of abuse among child with mph


rx_case <- merge(a9_5,bc1_1,by ="Reference.Key.",all.x=T)%>% 
  filter(Rxst<=edate1,Rxed>=sdate)%>% 
  filter(duplicated(Reference.Key.)==F)
length(unique(afinal_2$Reference.Key.))
...
test <- rx_case %>% 
  filter(is.na(admdt)|admdt>edate1)
sum(test$edate1-test$sdate+1)/365.25  #297549.8
test1 <- rx_case %>% 
  filter(admdt>=sdate,admdt<=edate1) %>% 
  filter(duplicated(Reference.Key.)==F)
sum(test1$admdt-test1$sdate+1)/365.25#3810.114
1064/(297549.8+3810.114)*1000


nnn <- test1 %>% 
  filter(!Reference.Key.%in%afinalb$Reference.Key.)


nnn1 <- a9_5 %>% 
  filter(Reference.Key.==1400119)
