...
fmla <- paste(event~Rxst0+Rxst1+Rxst2+Rxstw+c_i_under40_s+c_i_40to50_s+c_i_50to60_s+c_i_60to70_s+c_i_up70_s+age+season, "+", "strata(indiv)", 
              "+", "offset(log(interval))")
...
mod <- clogit(formula = fmla1, data = formatdata_all2 %>% filter(Rxstw!=1))
summary(mod)
...
formatdata_all3$rx[which(formatdata_all3$Rxstw==1)] <- "3.POST"
...
#705 CASE with outcome in non-expo
test <- formatdata_all2 %>% 
  filter(event==1&Rxst0==0&Rxst1==0&Rxst2==0)

length(unique(test$indiv)) #705
table(formatdata_all3$rx_fac)


test1 <- afinal2 %>% 
  filter(Reference.Key.%in%test$indiv) %>% 
  filter(Rxed<admdt) %>% 
  arrange(Reference.Key.,desc(Rxst)) %>% 
  filter(duplicated(Reference.Key.)==F) %>% 
  mutate(diffff=admdt-Rxed) #162

hist(test1$diffff,
     main="Duration from the latest Rx end to the event date",
     xlab="Day(s)",
     breaks=60,
     xlim=c(0,1000),
     ylim = c(0,40))
